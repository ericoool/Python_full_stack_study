...
# 得到邮件的原始内容
# 这个过程主要负责从MDA到MUA的下载,并使用Parser粗略解析
def getMsg():
    # 准备相应信息
    email = "191169906@qq.com"
    # 邮箱的授权码
    pwd = "qoyctsoasxdfbigd"
    ...
    index = len(mails)
    # retr负责返回一个具体索引号的一封邮件的内容,此内容不具可读性
    # lines存储邮件的最原始文本的每一行
...
def parseMsg(msg, indent=0):
    ...
    if indent == 0:
        for header in ['From', 'To', 'Subject']:
            ...
            if value:
                # Subject中的内容直接解码就可以,它是字符串类型
                if header == 'Subject':
                    value = decodeStr(value)
                # 如果是From和To字段,则内容大概是"我的邮箱<xxx@qq.com>"这种格式
                else:
    ...
    # 下面代码关注邮件内容本身
    # 邮件内容中,有可能是multipart类型,也可能是普通邮件类型
    # 下面的解析使用递归方式
    if (msg.is_multipart()):
        ...
        # enumerate函数是内置函数
        # 作用是将一个列表,此处是parts,生成一个有索引和parts原内容构成的新列表
...
def guessCharset(msg):
    ...
    if charset is None:
        # 找到内容类型,病转换成小写
        content_type = msg.get("Content-ytpe", "").lower()
        pos = content_type.find("charset=")
        if pos >= 0:
            # 如果包含charset,则内容形如 charset=xxxx
